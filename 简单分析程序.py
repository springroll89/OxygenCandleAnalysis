    ...
    if report_files:
        ...
        try:
            ...
            try:
                ...
                # 查找平均值异常分析行
                if '类型' in anomaly_df.columns:
                    avg_anomaly = anomaly_df[anomaly_df['类型'].astype(str).str.contains('平均值', na=False)]
                    if not avg_anomaly.empty:
                        row = avg_anomaly.iloc[0]
                        sample_data['异常次数'] = int(row.get('异常次数', 0))
                        sample_data['异常持续时间(秒)'] = float(row.get('异常持续时间(秒)', 0))
                        sample_data['流量差异'] = float(row.get('流量差异', 0))
                        print(f"  ✅ 异常次数: {sample_data['异常次数']}")
                    else:
                        sample_data['异常次数'] = 0
