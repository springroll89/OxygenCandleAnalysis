...
# 打印异常数据统计，帮助调试
print("\n异常数据统计：")
print(f"  有异常的样品数: {len(df[df['异常次数_数值'] > 0])}")
if len(df[df['异常次数_数值'] > 0]) > 0:
    print(f"  异常次数范围: {df['异常次数_数值'].min():.0f} - {df['异常次数_数值'].max():.0f}")
    print(f"  平均异常次数: {df[df['异常次数_数值'] > 0]['异常次数_数值'].mean():.1f}")
    
    # 显示前5个有异常的样品
    print("\n  有异常的样品示例:")
    anomaly_samples = df[df['异常次数_数值'] > 0].head(5)
    for idx, row in anomaly_samples.iterrows():
        print(f"    {row['样品编号']}: {row['异常次数_数值']:.0f}次异常")

# 异常得分：基础10分，扣分规则（基于新理解调整）：
# - 每个异常扣1分（异常次数就是数据行数）
# - 累计持续时间每20秒扣1分
# - 累计流量差异每0.5扣1分
def calc_anomaly_score(anomaly_count, anomaly_time, flow_diff):
    # 基础分10分
    score = 10.0
    
    # 异常次数扣分（每个异常扣1分）
    if anomaly_count > 0:
        score -= anomaly_count * 1.0
        
    # 累计异常时间扣分（每20秒扣1分）
    if anomaly_time > 0:
        score -= anomaly_time / 20
...
print("📊 异常扣分影响分析：")
print("-" * 60)
no_anomaly = df[df['异常次数_数值'] == 0]
has_anomaly = df[df['异常次数_数值'] > 0]
print(f"  无异常样品: {len(no_anomaly)}个，平均异常得分: {no_anomaly['异常得分'].mean():.1f}/10分")
print(f"  有异常样品: {len(has_anomaly)}个，平均异常得分: {has_anomaly['异常得分'].mean():.1f}/10分")
if len(has_anomaly) > 0:
    print(f"  异常对总分的平均影响: -{(10 - has_anomaly['异常得分'].mean()):.1f}分")
