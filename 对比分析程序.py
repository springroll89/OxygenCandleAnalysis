...
fig.suptitle(f'Oxygen Candle Analysis Report - {timestamp}', fontsize=16, fontweight='bold')
...
ax1.set_title('TOP10 Sample Ranking')
...
temp_levels_en = ['Standard', 'Low Temp', 'Mid Temp', 'High Temp']
avg_scores = []
...
bars2 = ax2.bar(temp_levels_en, avg_scores, color=colors2)
ax2.set_ylabel('Average Score')
ax2.set_title('Average Score by Temperature')
...
# 只绘制有数据的箱线图
valid_data = [(d, l, c) for d, l, c in zip(temp_data, temp_levels_en, colors2) if len(d) > 0 and any(d)]
if valid_data:
...
ax3.set_title('Compliance Rate Distribution')
ax3.grid(axis='y', alpha=0.3)

# 图4：样品数量统计
ax4 = axes[1, 1]
counts = [len(df[df['温度等级'] == temp]) for temp in temp_levels]
valid_temps = [(t, c, col) for t, c, col in zip(temp_levels_en, counts, colors2) if c > 0]
if valid_temps:
...
ax4.set_title(f'Sample Distribution (Total: {len(df)})')
